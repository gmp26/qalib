...
module.exports = (problems) ->
  ...
  # binomial theorem
  problems.makeBinomial2 = makeBinomial2 = ->
    p = new poly(1)
    p[0] = rand(1,5)
    p[1] = randnz(6 - p[0])
  ...
  problems.makePolyInt = makePolyInt = ->
    A = rand(-3, 2)
    B = rand(A + 1, 3)
  ...
  problems.makeTrigInt = makeTrigInt = ->
    a = rand(0, 7)
  ...
  # Vectors
  problems.makeVector = makeVector = ->
  ...
  problems.makeLinesEq = makeLinesEq = ->
  ...
  problems.makeLineParPerp = makeLineParPerp = ->
  ...
  problems.makeCircleEq = makeCircleEq = ->
    r = rand(2,7)
    a = rand(6)
    b = rand(6)
  ...
  problems.makeCircLineInter = makeCircLineInter = ->
  ...
  problems.makeFactor = makeFactor = ->
  ...
  problems.makeQuadratic = makeQuadratic = ->
  ...
  problems.makeComplete = makeComplete = ->
  ...
  problems.makeBinExp = makeBinExp = ->
    a = rand(1,3)
    b = randnz(2)
    n = rand(2, 5)
    m = rand(1, n - 1)
  ...
  problems.makeStationary = makeStationary = ->
  ...
  problems.makeTriangle = makeTriangle = ->
  ...
  problems.makeCircle = makeCircle = ->
    r = rand(2, 8)
    bot = rand(2, 9)
    top = rand(1, 2 * bot - 1)
  ...
  problems.makeSolvingTrig = makeSolvingTrig = ->
    A = pickrand(1,3,4,5)
  ...
  problems.makeVectorEq = makeVectorEq = ->
    a = new vector(3)
    a.setrand(6)
    b = new vector(3)
    b.setrand(6)
  ...
  problems.makeModulus = makeModulus = ->
    parms = 0
    fn = 0
    data = []
    graph = null
  ...
  problems.makeTransformation = makeTransformation = ->
  ...
  problems.makeComposition = makeComposition = ->
  ...
  problems.makeParametric = makeParametric = ->
    p = new poly(rand(1, 2))
    p.setrand(2)
  ...
  problems.makeImplicit = makeImplicit = ->
  ...
  problems.makeChainRule = makeChainRule = ->
  ...
  problems.makeProductRule = makeProductRule = ->
    fns = new Array("\\ln(z)", "\\csc(z)", "\\sec(z)", "\\sin(z)", "\\tan(z)", "\\cos(z)")
    difs = new Array("\\frac{y}{z}", " - y\\csc(z)\\cot(z)", "y\\sec(z)\\tan(z)", "y\\cos(z)", "y\\sec^2(z)", " - y\\sin(z)")
    even = new Array(-1, 1, - 1, 1, 1, - 1)
    which = rand(0, 5)
  ...
  problems.makeQuotientRule = makeQuotientRule = ->
  ...
  problems.makeImplicitFunction = makeImplicitFunction = ->
  ...
  problems.makeIntegration = makeIntegration = ->
  ...
  problems.makePowers = makePowers = ->
    res = new frac()
    q = ""
  ...
  problems.makeDiophantine = makeDiophantine = ->
    a = rand(1, 999)
    b = rand(1, 999)
  ...
  problems.makeDistance = makeDistance = ->
  ...
  problems.makeCircumCircle = makeCircumCircle = ->
    maxdist = 10
    ...
    X = (a1^2 + a2^2) * (b2 - c2) + (b1^2 + b2^2) * (c2 - a2) + (c1^2 + c2^2) * (a2 - b2)
    Y = (a1^2 + a2^2) * (c1 - b1) + (b1^2 + b2^2) * (a1 - c1) + (c1^2 + c2^2) * (b1 - a1)

    x = new frac(X, d)
    y = new frac(Y, d)

    sq = new sqroot((X - b1 * d)^2 + (Y - b2 * d)^2)
    # while sq.n > 1000
  ...
  problems.makeCArithmetic = makeCArithmetic = ->
  ...
  problems.makeCPolar = makeCPolar = ->
    if rand()
      z = Complex.randnz(6, 6)
    else
  ...
  problems.makeDETwoHard = makeDETwoHard = ->
    p = new poly(2)
    p.setrand(6)
    p[2] = 1
  ...
  problems.makeMatrixQ = makeMatrixQ = (dim, max) ->
  ...
  problems.makeMatrix2 = makeMatrix2 = -> makeMatrixQ(2, 6)
  problems.makeMatrix3 = makeMatrix3 = -> makeMatrixQ(3, 4)



  problems.makeTaylor = makeTaylor = ->
  ...
  problems.makePolarSketch = makePolarSketch = ->
  ...
  problems.makeFurtherVector = makeFurtherVector = ->
  ...
  problems.makeFurtherIneq = makeFurtherIneq = ->
  ...
  problems.makeSubstInt = makeSubstInt = -> /* Has issues with polys which are never in the domain of, say, arcsin worked around for now */
    p = new poly(rand(1, 2)); p.setrand(2)
  ...
  problems.makeRevolution = makeRevolution = ->
  ...
  problems.makeMatXforms = makeMatXforms = ->
    a = rand(0, 2)
    xfms = new Array(5)
  ...
  problems.makeDiscreteDistn = makeDiscreteDistn = ->
    # nparms = [2, 1, 1]
  ...
  problems.makeContinDistn = makeContinDistn = ->
  ...
  problems.makeHypTest = makeHypTest = ->
  ...
  problems.makeConfidInt = makeConfidInt = ->
    mu = rand(4)
    sigma = rand(1, 4)
    n = 2 * rand(6, 10)
  ...
  problems.makeChiSquare = makeChiSquare = ->
    tableN.populate()
  ...
  problems.makeProductMoment = makeProductMoment = ->
    n = rand(6, 12)
    mu = [rand(4), rand(4)]
