...
module.exports = (helpers) ->
  ...
  helpers.lincombination = lincombination = (a,b) ->
  ...
  helpers.distrand = distrand = (n, min, max) ->
  ...
  # If max omitted, then [-|min|, |min|].
  # If both omitted, then [1, n]
  helpers.distrandnz = distrandnz = (n, min, max) ->
  ...
  helpers.pickrand = pickrand = -> pickrand.arguments[rand(0, pickrand.arguments.length - 1)]



  # Levi-Civita symbol on {n, n+1, n+2} for integer n
  helpers.epsi = epsi = (i,j,k) -> (i - j) * (j - k) * (k - i) / 2

  helpers.rPad = rPad = (n, len, char) ->
    s = n.toString()
    for i from s.length+1 to len
      s += char
    s

  #String::repeat = (num) -> (new Array num + 1).join this
  helpers.repeat = repeat = (num, s) -->
    (for til num
      s
    ).join ""
  ...
  helpers.signedNumber = signedNumber = (x) -> if x > 0 then "+" + x else x.toString!



  # Simplifies a root of the form (a + b sqrt(c)) / d
  # Returns a LaTeX formatted string
  helpers.simplifySurd = simplifySurd = (a,b,c,d) ->
    sq = new sqroot(c)
    B = sq.a * b
    C = sq.n
